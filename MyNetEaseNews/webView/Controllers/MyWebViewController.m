...
- (void)setupWebViewByData:(id)data {
  ...
  if (data!= nil) {
    ...
    NSMutableString *allTitleStr =[NSMutableString stringWithString:@"<style type='text/css'> p.thicker{font-weight: 900}p.light{font-weight: 0}p{font-size: 108%}h2 {font-size: 130%}h3 {font-size: 80%}</style> <h2 class = 'thicker'>xukun</h2><h3>hehe    lala</h3>"];
    ...
    if ([videoArray count]) {
      ...
      for (NSDictionary *videoDic in videoArray) {
        ...
        NSString *videoStr = [NSString stringWithFormat:@"<embed height='%f' width='%f' src='%@' />",self.view.frame.size.width*180/280.0*0.9,self.view.frame.size.width*0.9,videoin.url_mp4];
